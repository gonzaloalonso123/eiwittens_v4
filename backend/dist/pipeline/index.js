...
let isRunning = false;
export async function scrapeAndPush() {
    if (isRunning) {
    ...
    try {
        ...
        if (warnings.length > config.ALLOWED_WARNINGS) {
            await sendErrorAlert(`Too many warnings after scrape run: ${warnings.length} products flagged. ` +
        ...
        console.log(`[pipeline] Done — ${result.scrapedCount} products, ` +
            `${result.warningCount} warnings, ${result.durationMs}ms`);
        return result;
    }
    catch (err) {
        await sendErrorAlert(`Pipeline failed: ${err.message}`);
        throw err;
    }
    finally {
        isRunning = false;
...
async function persistAll(products) {
    for (const product of products) {
