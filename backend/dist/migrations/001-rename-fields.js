...
async function migrate() {
    ...
    for (const doc of snapshot.docs) {
        ...
        const oldScraper = data['scraper'];
        if (Array.isArray(oldScraper) && oldScraper.length > 0) {
            const needsTransform = oldScraper.some((a) => ('selector' in a && !('selectorType' in a)) || ('xpath' in a && !('selectorValue' in a)) || !a.id);
            if (needsTransform) {
        ...
        if (!('out_of_stock' in data))
            updates['out_of_stock'] = false;
        if (!('enabled_top10' in data))
            updates['enabled_top10'] = true;
        if (!('only_in_store' in data))
            updates['only_in_store'] = false;
...
//# sourceMappingURL=001-rename-fields.js.map