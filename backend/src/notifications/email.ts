...
export async function sendWarningDigest(
    warnings: Warning[],
    products: Product[],
): Promise<void> {
...
function buildDigestHtml(warnings: Warning[], products: Product[]): string {
        ...
        provisionalProducts.length > 0
         ...
         <table border="1" cellpadding="4">
           ...
           ${provisionalProducts
                .map(
                    (p) =>
                        `<tr><td>${p.name}</td><td>€${p.price}</td><td>€${p.provisional_price}</td></tr>`,
                )
                .join('')}
         </table>
