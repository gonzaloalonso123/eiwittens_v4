...
function pricePerElementGram(product: Product, elementPer100g: number): number | null {
    if (!product.amount || !product.price) return null;
    const elementGrams = product.amount * (elementPer100g / 100);
    if (elementGrams === 0) return null;
    return product.price / elementGrams;
}

function getComparablePrice(product: Product): number | null {
...
export function applySecurityCheck(
    newProducts: Product[],
    oldProducts: Product[],
): Product[] {
    return newProducts.map((newProduct) => {
        ...
        if (newPpg === null || oldPpg === null) return newProduct;

        const isSuspiciouslyLow = newPpg < oldPpg * (1 - PRICE_DROP_THRESHOLD_PERCENT / 100);

        if (isSuspiciouslyLow) {
            console.warn(
                `[security] Suspicious price drop for "${newProduct.name}": ` +
